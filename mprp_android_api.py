...
def get_requested_items_details(loggedInUser):
	...
	if record:
		...
		if cdrecord:
			requestedItemRecords = frappe.db.sql(""" select ritms.item_code, ritms.upstream_warehouse from `tabControlDocument Item` ritms where ritms.parent = %s""",(cdrecord[0][0]))
			...
			if requestedItemRecords:
				for item in requestedItemRecords:
					...
					upstream_warehouse = item[1]
								
					if itemDetails:
						constructedJson = {}
						availQnty = 0.0
						constructedJson ['item_code'] =  itemDetails[0][0]
						constructedJson ['warehouse'] = upstream_warehouse
	...
	if record:
		...
		if cdrecord:
			...
			for jsonobject in data['requiredItems']:
				...
				upstream_warehouse = jsonobject['src_warehouse']
