...
@frappe.whitelist()
def get_issue_material_from_stock_details(loggedInUser):
	outerJson = []
	...
	if record:
		cdrecord = frappe.db.sql("""select cd.name, cd.warehouse, cd.upstream_warehouse, cd.downstream_warehouse from `tabControlDocument` cd where cd.user =%s and cd.docstatus = 1 and cd.is_active = 1 and cd.is_default = 1""",(record[0]))
		if cdrecord:
			issueMaterialItemsFromStock = frappe.db.sql(""" select issuedsitms.issue_these_items_downstream, issuedsitms.downstream_warehouse from `tabControlDocument Issues Downstream` issuedsitms where issuedsitms.parent = %s""",(cdrecord[0][0]))
			warehouse = cdrecord[0][1]
			upstream_warehouse = cdrecord[0][2]
			#downstream_warehouse = cdrecord[0][3]
			for itemRecord in issueMaterialItemsFromStock:
				if itemRecord:
					ItemCode = itemRecord[0]
					downstream_warehouse_list = itemRecord[1]
					detailsOfItem = frappe.db.sql("""select it.has_batch_no, it.has_serial_no, it.stock_uom from `tabItem` it where it.item_code = %(itemCode)s""",{'itemCode':ItemCode})
					if detailsOfItem:
						
						#do something here
						hasBatchNumber = detailsOfItem[0][0]
						if hasBatchNumber == 1:
							batchRecords = getBatchNos(ItemCode, warehouse)
						else:
							batchRecords = None
						hasSerialNumber = detailsOfItem[0][1]
						if hasSerialNumber == 1:
							serialNoRecords = getSerialNoRecords(ItemCode, warehouse)
						else:
							serialNoRecords = None
						item_stock_uom = detailsOfItem[0][2]
						availQnty = 0.0
						if get_latest_stock_qty(ItemCode,warehouse):
							availQnty = get_latest_stock_qty(ItemCode,warehouse)						
						#now construct the inner json for each of the items to be issued
						itemsJson = {}
						
						itemsJson['item_code'] = ItemCode
						itemsJson['downstream_wh_list'] = downstream_warehouse_list.split("\n")
						itemsJson['stock_uom'] = item_stock_uom
						itemsJson['has_batch_no'] = hasBatchNumber
						itemsJson['has_serial_no'] = hasSerialNumber
						itemsJson['available_quantity'] = availQnty
						itemsJson['batch_nos'] = batchRecords
						itemsJson['serial_nos'] = serialNoRecords
						outerJson.append(itemsJson)
					else:
